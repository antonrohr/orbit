...
ProfilingTargetDialog::ProfilingTargetDialog(
  ...
  TargetConfiguration target = std::move(target_configuration_opt.value());
  target_configuration_opt = std::nullopt;

  if (std::holds_alternative<StadiaTarget>(target)) {
    ui_->stadiaWidget->Start(ssh_connection_artifacts,
                             std::move(std::get<StadiaTarget>(target).connection_));
  } else {
    ui_->stadiaWidget->Start(ssh_connection_artifacts);
  }
...
void ProfilingTargetDialog::OnProcessListUpdate(ProcessManager* process_manager) {
  std::vector<ProcessInfo> process_list = process_manager->GetProcessList();

  QMetaObject::invokeMethod(QCoreApplication::instance(),
                            [this, process_list = std::move(process_list)]() {
